...
const CombinedSelector = require('./combined-selector');
 ...
 *    shallowCopy({}) => {}
 */
function shallowCopy(obj, assign = 'assign') {
  if (assign) {
    /* do nothing */
  }
  const result = {};
...
function isEmptyObject(obj) {
  ...
  for (let i = 0; i < keys.length; i += 1) {
    if (Object.hasOwn(obj, keys[i])) return false;
  }
  return true;
}
